function time = matCS_step_separation_time (step);

% function time = matCS_step_separation_time (step);
%
% Return the time of "event separation" of a given step (properly wraps multi-day IONIC measurements to the date given in the data file).
%
% INPUT:
% step: step struct (see matCS_read_step), or array of step structs
%
% OUTPUT:
% time: time of "event separation" (Matlab datenum)

if ( length(step) > 1 )
    time = repmat (NaN,size(step));
    for i = 1:length(step)
        time(i) = matCS_step_separation_time(step(i));
    end
else
	if ~isempty(step)
        ...
        t = mod (step.separation_time,24*60*60);
        ...
        day_separation = floor (step.separation_time/60/60/24); % start of measurement
        day_pumpout = floor (step.pumpout_time/60/60/24); % end of measurement
        if day_pumpout > day_separation % measurement was running over midnight
        	time = time - (day_pumpout-day_separation); % subtract offset
        	warning (sprintf('matCS_step_separation_time: measurement was running over midnight (file: %s). The separation time was adjusted by assuming the date given in the file corresponds to the end of the measurement. Please be careful...',step.file));
        end

	else
		warning ("matCS_step_separation_time: step is empty! Returning time = NaN...")
		time = NaN;
	end
end
